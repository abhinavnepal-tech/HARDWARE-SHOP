...
function handlePDFError(operation, error) {
    ...
    errorDiv.innerHTML = `
        ...
        Unable to ${operation} PDF. <a href="book.pdf" target="_blank" style="color: white; text-decoration: underline;">Try opening directly</a>
    `;
    ...
    try {
        ...
        if (pdfEmbed) {
            try {
                ...
                const printWindow = window.open('book.pdf', '_blank');
                if (printWindow) {
                    printWindow.onload = function() {
                        try {
                            printWindow.print();
